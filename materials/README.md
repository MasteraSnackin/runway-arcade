# Runway Arcade Project Materials

This folder contains collateral for explaining, presenting, judging, and handing over the Runway Arcade hackathon project.

Current source context:

- Product: `Runway Arcade - Xero Finance Co-Pilot`
- Current preview: `https://lovable.dev/preview/9PwGxwRjJf0sZ7tbEsUsruPWSBJYRozm`
- Local mirror: `../runway-arcade-local/`
- Local URL: `http://localhost:4173/`
- Current revision: `d6b609f0030ef666ff7958e08c7838e3cfc3ee20`
- Material date: 2026-07-04

## Contents

| File | Use |
| --- | --- |
| [project-one-pager.md](project-one-pager.md) | Short product summary for stakeholders, judges, and teammates. |
| [hackathon-submission.md](hackathon-submission.md) | Ready-to-adapt hackathon submission copy. |
| [pitch-deck-outline.md](pitch-deck-outline.md) | Slide-by-slide pitch structure with speaker notes. |
| [judge-demo-runbook.md](judge-demo-runbook.md) | Practical demo script, timings, setup, and recovery plan. |
| [technical-handover.md](technical-handover.md) | Engineering handover for continuing beyond the mirrored bundle. |
| [qa-and-evidence-checklist.md](qa-and-evidence-checklist.md) | Pre-demo checks and evidence checklist. |
| [product-copy-bank.md](product-copy-bank.md) | Taglines, short descriptions, UI copy, and FAQ text. |
| [tiger-mum-cashflow-persona.md](tiger-mum-cashflow-persona.md) | Optional strict-but-caring co-pilot persona and prompt layer. |
| [project-provenance-and-update-log.md](project-provenance-and-update-log.md) | Timeline of what was cloned, synced, verified, and what remains unknown. |
| [screenshots/runway-arcade-dashboard.png](screenshots/runway-arcade-dashboard.png) | Current dashboard screenshot from the local mirror. |
| [screenshots/runway-arcade-judge-demo.png](screenshots/runway-arcade-judge-demo.png) | MAYDAY / Judge demo screenshot from the local mirror. |

## Positioning

Runway Arcade turns Xero-style finance data into an evidence-backed cashflow cockpit for small businesses. It shows cash runway, near-term hazards, overdue invoice recovery opportunities, route plans, approval gates, and an AI co-pilot that explains its recommendations.

The current local app is simulation-only. It does not make real Xero writes, send payment instructions, or contact customers.
